- **AGENTS:**
  - [bbugyi200.athena.zy--2](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.zy.md)

#fork:zy--1 %model:sonnet %effort:xhigh
...
**Why this was monitored:** check-full aborted early on a known pre-existing symvision
failure (bead sase-ld); run the remaining gates it never reached, especially the full
test suite (test-cost), to actually verify the phantom_starting_agent_rows changes
...
Report pass/fail for each remaining gate (toobig lint, validate,
validate-committed-plans, full test suite via test-cost, selection-health). If the full
test suite fails, diagnose whether it is caused by the phantom_starting_agent_rows
changes (src/sase/ace/tui/models/_dedup.py, agent_panels.py, agent_panel_index.py,
artifact_files.py, src/sase/monitor/claims.py, src/sase/monitor/start.py, and the many
test files updated to give STARTING-status test fixtures a recent start_time instead of
a hardcoded past date) versus an unrelated flake; fix it if caused by this change.
Combine with the earlier check-full result (all lint gates passed except the
known/expected symvision finding on stream_and_parse_messages_json_output, bead sase-ld)
into one final summary for the user confirming the phantom_starting_agent_rows
