- **AGENTS:**
  - [bbugyi200.athena.0d6--3](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0d6.md)

#fork:0d6--2 %model:sonnet %effort:xhigh
...
**Why this was monitored:** check-full aborted early at the pre-existing 'SASE
validation' (init memory --check) gate before reaching the full test suite; confirmed
this validate failure is identical on unmodified master (stashed plan changes, reran,
same +2-2 README drift), so run the full test-cost suite directly to get authoritative
full-suite results for the disabled_region_launch_expansion plan changes.
...
Report a pass/fail summary. Known pre-existing/unrelated failures (already confirmed
identical on unmodified master):
tests/test_bead/test_cli_golden.py::test_bead_cli_golden_contract[list_full],
[list_json], [list_json_limit], [list_implicit_closed_json], [show_json],
[show_phase_json];
...
If just test-cost shows exactly these 8 failures (or a subset) and otherwise passes,
reply to the user with a final implementation summary for the
disabled_region_launch_expansion plan
(@plan:202608/disabled_region_launch_expansion.md): summarize what changed in
src/sase/main/query_handler/_embedded_workflows.py,
src/sase/axe/run_agent_runner_setup.py, src/sase/xprompt/_parsing.py, and
src/sase/xprompt/_parsing_vcs_tags.py, note all lint/fmt gates in check-full passed
cleanly, note the pre-existing SASE-validation (memory README drift) and 8 pre-existing
test failures as confirmed-unrelated (do NOT fix them — they require explicit user
permission to touch sase/memory files), and note this task is otherwise ready to land.
If NEW failures appear beyond those 8, diagnose whether they relate to the plan's
changed files or their new tests (tests/test_disabled_regions.py,
tests/test_fork_workflow.py, tests/test_run_agent_runner_setup.py,
tests/test_xprompt_vcs_tag_parsing.py), fix them, and re-run just test-cost through
another /sase_monitor to confirm before replying to the user. %xprompts_enabled:true
