- **AGENTS:**
  - [bbugyi200.athena.sase-u6.4--2](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-u6.4.md)

#fork:sase-u6.4--1 %model:@small
...
**Why this was monitored:** Run the full verification lane after Artifacts visual
goldens and cost-budget baseline update for bead sase-u6.4
...
Continue bead sase-u6.4 after the monitored `just check-full` completes. Work done in
this workspace: added Artifacts pane-description PNG coverage in
`tests/ace/tui/visual/test_ace_png_snapshots_artifacts_descriptions.py`, added four new
Artifacts description PNG goldens, rebaselined the Artifacts PNG corpus for the pane
brief row, adjusted the Files-pane visual sentinels that the new row pushed out of the
viewport, and recalibrated hard CPU entries in
`tests/perf/baselines/test_cost_budgets.json` using
`tools/check_test_cost_budgets --suggest --history 8` after repeated full-lane cost
budget failures. Verified so far:
`just test-visual -- tests/ace/tui/visual/test_ace_png_snapshots_artifacts*.py --sase-update-visual-snapshots`;
isolated split update/assertion for artifacts_split_wide_120x40;
`just test-visual -- tests/ace/tui/visual/test_ace_png_snapshots_artifacts*.py`;
`just check`;
`just test tests/test_test_cost_committed_budgets.py tests/test_test_cost_budgets.py`;
`.venv/bin/python tools/check_test_cost_budgets --recording /home/bryan/.sase/test-selection/gh_sase-org__sase/timings/cost/20260826T162519Z-1371849.json`.
If `just check-full` passed, run `sase bead epic-symbols sase-u6.4`; if it reports no
entries, close only this bead with
`sase bead close sase-u6.4 --note "Verified Artifacts description PNG coverage and Artifacts rebaseline; Artifacts visual subset passed; cost-budget checks passed after baseline recalibration; just check passed; just check-full passed."`.
Do not close the parent epic or any ancestor. If `just check-full` failed, fix only the
relevant failures, rerun appropriate verification including the full lane as required,
then run epic-symbols and close only `sase-u6.4` when clean. Finish with the required
SASE final declaration. %xprompts_enabled:true
