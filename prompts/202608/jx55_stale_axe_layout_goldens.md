- **PLAN:**
  [202608/jx55_stale_axe_layout_goldens.md](https://github.com/sase-org/sase--plans/blob/main/202608/jx55_stale_axe_layout_goldens.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-jx.5.5.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-jx.5.5.land.md)

You are the land agent for epic bead sase-jx.5.5: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-jx.5.5` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-jx.5.5, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   `sase bead close sase-jx.5.5 --note "<what you verified in steps 1-2>"`. AFTER
   closing, run `just symvision` if available (epic-symbol whitelist entries for
   sase-jx.5.5 expire at close) and remove the stale entries and unused code it reports.
