- **AGENTS:**
  - [bbugyi200.athena.0bw--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0bw.md)

#fork:0bw--code %model:gpt-5.5 %effort:xhigh
...
**Why this was monitored:** Run the required exhaustive verification after implementing
the approved Stitch type filter plan
...
Inspect the just check-full monitor result. If it failed, fix the failure and rerun the
appropriate verification. If it passed, note that this turn already completed: just
install, focused Python pytest for the VCS/query/Stitches surfaces (277 passed), just
rust-check for the linked sase-core repo, and just check (passed, scoped lane escalated
to full suite). Then run the SASE final declaration as the last action and reply to the
user with a concise implementation and verification summary. %xprompts_enabled:true
