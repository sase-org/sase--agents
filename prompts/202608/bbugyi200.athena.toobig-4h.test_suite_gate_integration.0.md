- **AGENTS:**
  - [bbugyi200.athena.toobig-4h.test_suite_gate_integration.0--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.toobig-4h.test_suite_gate_integration.0.md)

#fork:toobig-4h.test_suite_gate_integration.0 %model:sonnet %effort:xhigh
...
**Why this was monitored:** Install deps then verify the test_suite_gate_integration.py
split with just check
...
Report the just check results for the test_suite_gate_integration.py split (into
test_suite_gate_scoped_integration.py and test_suite_gate_scaled_integration.py) plus
docs/development.md reference update. If it failed, diagnose and fix, then confirm
clean, then reply to the user summarizing the split and verification result.
