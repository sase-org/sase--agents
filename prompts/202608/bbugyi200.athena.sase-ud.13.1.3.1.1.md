- **AGENTS:**
  - [bbugyi200.athena.sase-ud.13.1.3.1.1--2](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-ud.13.1.3.1.1.md)

#fork:sase-ud.13.1.3.1.1 %model:sonnet %effort:high
...
**Why this was monitored:** Validate the gate-contract guard tests for bead
sase-ud.13.1.3.1.1 after fixing a ruff formatting issue in
...
If just check passed: run `sase bead epic-symbols sase-ud.13.1.3.1.1` (expect no
leftover --epic-symbol entries since this is a test-only phase; if any exist, resolve or
re-key per the phase instructions before closing), then run
`sase bead close sase-ud.13.1.3.1.1 --note "<summary of what was verified, e.g. added N guard tests in tests/test_agent_loader_status_override_gate_shell_family.py pinning the gate-shell family projection contract per the plan gate-contract cases; just check passed>"`,
then reply to the user with a short summary. If just check still fails on the new test
file itself: the plan states these cases must already pass on the tree at commit
a646bdaf6, so a failure means the fixture construction has a bug (not production code) —
inspect the failure, fix ONLY
tests/test_agent_loader_status_override_gate_shell_family.py, rerun just check, and only
close the bead once green. If just check fails for unrelated pre-existing reasons, use
judgment per plan risk R-5 (task bead sase-j0 tracks pre-existing check-full failures on
master) — just check (not check-full) is the required gate. Do NOT close any ancestor
bead (sase-ud.13.1.3.1, sase-ud.13.1.3, sase-ud.13.1, sase-ud.13, sase-ud). Record any
discovered follow-up work as a PROPOSED FOLLOW-UP note via `sase bead note` rather than
creating new beads. Finish with the SASE final declaration flow. %xprompts_enabled:true
