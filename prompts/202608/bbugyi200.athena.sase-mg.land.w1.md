- **AGENTS:**
  - [bbugyi200.athena.sase-mg.land.w1--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-mg.land.w1.md)

#fork:sase-mg.land.w1--code %model:grok-4.6 %effort:xhigh
...
**Why this was monitored:** unify_var_get scoped selection escalated (rename-or-delete
of test_var_show.py); verify the full lint+test lane
...
- The plan is implemented: sase var show is gone; sase var get has three modes (current
  snapshot, quoted <agent_name> snapshot, selectors). Skill source,
  docs/configuration.md, docs/cli.md, docs/xprompt.md, and landing tests were updated.
  Do not deploy skills.
- A one-line ruff format of
  tests/ace/tui/test_proc_producer_inventory.py:_proc_type_from_keyword was applied
  only because HEAD already failed fmt-py-check and blocked just check. That leftover
  belongs to epic sase-m9.3.1 (already has DISCOVERED ISSUE notes). Do not expand that
  change.
...
- If a failure is pre-existing/unrelated, corroborate the owning bead/epic; do not mix
  extra fixes.
- Re-run the smallest relevant verification. Use /sase_monitor again for another just
  check-full if needed.

If just check-full passed:

- Reply to the user with a standalone summary of what was implemented (three get modes,
  show removed, skill/docs/tests, verification outcome). Mention that skills were
  previewed with sase skill init --diff and not deployed.
