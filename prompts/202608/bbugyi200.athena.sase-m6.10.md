- **AGENTS:**
  - [bbugyi200.athena.sase-m6.10--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-m6.10.md)

#fork:sase-m6.10--plan %model:gpt-5.5 %effort:xhigh
...
**Why this was monitored:** sase-m6.10 remaining verification: fmt/ruff/mypy plus full
test suite. just check-full is blocked by pre-existing stale --epic-symbol lint for
closed beads sase-n9 and sase-na.4.
...
You are finishing sase-m6.10 (conform phase). The bead is reserved for you; do not set
status by hand.

Work already landed in this workspace (do not redo it): SHELL and STATUS_COUNTERS are
real derivation rules; conformance is parametrized over every resolved sub-tab and
asserts contract-declared keys resolve to the named action; ACE surfaces
missing_ref_provider via a warning tab icon, header chip, and
artifacts_provider_diagnostics(); relation status glyphs are no longer Patch-only;
docs/artifacts_pane_contract.md plus mkdocs/ace/cli/perf_runbook updates;
stitches.next/prev/up10 have a documented 20ms carve-out.

1. Read the monitor result. If fmt/ruff/mypy/tests failed on this phase's files, fix
   them and re-run the failing slice.
2. Ignore just-check lint(symvision) failures that only name stale --epic-symbol entries
   for closed beads sase-n9 and sase-na.4 — already recorded as PROPOSED FOLLOW-UP on
   sase-m6.10.
3. When verification is acceptable, close ONLY this bead: sase bead close sase-m6.10
   --note "<what you verified>". Do NOT close the parent epic or any ancestor.
4. Reply to the user with what the conform phase delivered and what was verified. Do not
   create beads; further follow-up stays as PROPOSED FOLLOW-UP notes on sase-m6.10.
