- **AGENTS:**
  - [bbugyi200.athena.0fc--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0fc.md)

#fork:0fc %model:@small
...
**Why this was monitored:** Verify the approved axe chop summary contract implementation
...
Continue the approved implementation for plan 202608/axe_chop_summary_contract.md in
this workspace. Before the monitor handoff, the code changes were: gate_shell_reclaim
now returns runtime.emit_summary(...) with reason=no_pending_gate_shells when
scanned==0; stale_running_cleanup avoids importing sase.monitor.start when
skip_monitor_claims=True while preserving ace-monitor claim skipping; regression tests
were added for both and for builtin chop contract shape. Verification already completed:
.venv/bin/pytest tests/test_axe_chop_output_contract.py
tests/test_stale_running_cleanup.py passed; just check passed; manual reclaim helper
output was {scanned: 2, answered: 0, stopped: 0, timed_out: 0, lost: 0, errors: 0};
manual end-to-end gate_shell_reclaim script run exited 0 and wrote schema_version=1
status=ok. Follow-up notes were recorded on active epics sase-kp and sase-ud per
/sase_new_task policy, and the plans/beads sidecars were clean afterward. Inspect this
monitor result. If just check-full failed, fix the failure and rerun the necessary
checks. If it passed, do a final git status/diff sanity check, then use the mandatory
/sase_final flow and reply to the user with the concise outcome. %xprompts_enabled:true
