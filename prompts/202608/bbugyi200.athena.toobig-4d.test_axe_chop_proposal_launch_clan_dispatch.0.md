- **AGENTS:**
  - [bbugyi200.athena.toobig-4d.test_axe_chop_proposal_launch_clan_dispatch.0--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.toobig-4d.test_axe_chop_proposal_launch_clan_dispatch.0.md)

#fork:toobig-4d.test_axe_chop_proposal_launch_clan_dispatch.0--plan %model:sonnet
%effort:xhigh
...
**Why this was monitored:** Verify the test file split (clan dispatch tests) before
...
The user asked to split tests/test*axe_chop_proposal_launch_clan_dispatch.py (originally
808 lines) into files <=500 lines. It was split into:
tests/test_axe_chop_proposal_launch_clan_dispatch.py (clan promotion/declare unit
tests), tests/test_axe_chop_proposal_launch_clan_dispatch_wait.py (wait-relink unit
tests), tests/test_axe_chop_proposal_launch_clan_dispatch_e2e.py (end-to-end batch
tests), and tests/_axe_chop_proposal_launch_clan_dispatch_helpers.py (shared
constructor helpers, following this repo convention of tests/*<name>_helpers.py). just
install && just check just ran to verify. If it passed cleanly, briefly tell the user
the split succeeded (name the new files and line counts) and finish the turn normally
(the SASE final declaration flow handles committing). If it failed, read the failure
output, fix the issue in the new split files (likely an import or lint issue introduced
by the split), rerun just check inline (or via a fresh monitor if slow) until it passes,
then report to the user. %xprompts_enabled:true
