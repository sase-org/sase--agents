- **PLAN:**
  [202608/canonical_search_body_integration.md](https://github.com/sase-org/sase--plans/blob/main/202608/canonical_search_body_integration.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-e7.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-e7.land.md#member-plan)

You are the land agent for epic bead sase-e7: verify the epic is truly complete, integrate it with changes that landed
since it started, then close it out.

1. Verify. Run `sase bead show sase-e7` (children, linked plan file), review the epic bead's own notes, then run
...
2. Integrate. Changes committed since this epic started could not integrate with this epic's feature while it was
   incomplete. Find them (e.g. `git log` since the first commit mentioning sase-e7, excluding the epic's own commits; in
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them before closing. For each genuinely
   ...
   `sase bead close sase-e7 --note "<what you verified in steps 1-2>"`. AFTER closing, run `just symvision` if available
   (epic-symbol whitelist entries for sase-e7 expire at close) and remove the stale entries and unused code it reports.
