- **AGENTS:**
  - [bbugyi200.athena.0fs--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0fs.md)

#fork:0fs %model:gpt-5.5 %effort:xhigh
...
**Why this was monitored:** Run the landing verification for the approved AGENTS v2
...
Continue the AGENTS v2 annotation implementation in this same workspace. The source/test
changes are implemented, `.venv/bin/sase memory init --check` is clean,
`git diff --check` is clean, and inline `just check` passed with the scoped lane
escalating to the full suite. Inspect the monitored `just check-full` result. If it
passed, check `git status --short`, summarize the changes, and use `/sase_final` as the
last action before replying to the user. If it failed, fix only failures relevant to
this change, rerun needed verification, then use `/sase_final` before replying.
