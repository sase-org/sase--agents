- **AGENTS:**
  - [bbugyi200.athena.sase-s3.2--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-s3.2.md)

#fork:sase-s3.2--plan %model:grok-4.6 %effort:xhigh
...
**Why this was monitored:** sase-s3.2: exhaustive lint+full-suite after raising
sase-core-rs floor to 0.30.0
...
You are the sase-s3.2 follow-up after just check-full for python_monitor_integration
(bind the committed Python cleanup path to released sase-core-rs 0.30.0).
...
- tests/test_core_facade/test_agent_cleanup_facade.py: require installed schema 4 and
  fail if plan_agent_cleanup falls back to the Python planner on live-monitor and
  clan-sequential scenarios

Already verified:

- just install: package 0.30.0, agent_cleanup_wire_schema_version()==4, rust planner
  binding used
- 70 focused tests passed (facade/parity/python/targets, monitor persist, named-agent
  kill, TUI live-monitor kill, owner-cleanup)
- A first escalated just check had 17 failures from being behind origin (sase-s2.3
  archive stubs + contract manifest) plus a stale .venv sase-xprompt-lsp 0.29.9; origin
  was fast-forwarded and LSP rebuilt to 0.30.0; those tests then passed

Inspect just check-full. Do not automatically attribute any failure to this floor bump;
diagnose first. If a failure is unrelated, record PROPOSED FOLLOW-UP on sase-s3.2 and
still finish this phase only if the floor/parity work itself is sound.

If check-full passed (or leftover failures are proven unrelated and already noted):

1. Run `sase bead epic-symbols sase-s3.2`. If this phase still has --epic-symbol
   entries, resolve each symbol or re-key the Justfile line to a still-open bead (parent
   epic or a later phase).
2. Close only this bead: `sase bead close sase-s3.2 --note "<what you verified>"`.
   Example: "Raised sase-core-rs floor to >=0.30.0,<0.31.0; installed binding reports
   schema 4; Rust planner handles live-monitor selection/cascade without Python
   fallback; focused cleanup tests passed; just check-full passed."
3. Do NOT close parent epic sase-s3 or any ancestor. Do not set status by hand.
4. Do NOT create beads; discovered follow-up goes on this phase as
   `sase bead note sase-s3.2 'PROPOSED FOLLOW-UP: <one-line summary — detail>'`.
5. Use /sase_final as the last normal action. After a successful sase final submit, make
   no more file or repository changes.
