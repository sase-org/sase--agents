- **AGENTS:**
  - [bbugyi200.athena.0c3--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0c3.md)

#fork:0c3--code %model:gpt-5.5 %effort:xhigh
...
**Why this was monitored:** Verify green-master CI repair after /sase_hg_commit cleanup
and visual snapshot rebaseline
...
Inspect the just check-full result for the green_master_ci_for_v0_17_0 implementation.
Current local evidence before this monitor: just fmt passed; focused
stale-skill/parser/agy guard lane passed with 41 passed; SASE_PYTEST_WORKERS=1 just
test-visual passed with 788 passed, 1 skipped. Main repo has docs/test fixes plus
regenerated PNG goldens. Beads: closed sase-sj and sase-sm as fixed; left sase-sl ready
as the owner-approved memory follow-up; recorded +1 on closed sase-ca for
generated_skills memory drift. If just check-full fails, fix the reported issue(s),
rerun the needed verification, and avoid editing memory files unless the user explicitly
approves. If just check-full passes, review the diff, then finish the SASE
finalizer/commit workflow for this implementation. %xprompts_enabled:true
