- **AGENTS:**
  - [bbugyi200.athena.sase-pv.land--3](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-pv.land.md)

#fork:sase-pv.land--2 %model:opus %effort:max
...
**Why this was monitored:** Land verification for epic sase-pv, run 3: the two
check-full gates that never got a result. Run 1 (0j26dcy3j867) passed every lint gate on
this exact tree then died to a 15m idle timeout inside run_silent-wrapped test-cost; run
2 (vccf65gycnky) died at the flags gate on an external orphan flag bead. Running the
test lane unwrapped so its output is visible, with no idle timeout so a suite-gate wait
...
You are resuming the land of epic sase-pv. Verification and integration are DONE and the
close note is fully written at /tmp/sase-pv-land/close_note.txt. Only the close-out
remains.

WHY THIS RUN EXISTS. check-full run 1 (monitor 0j26dcy3j867) passed EVERY whole-repo
lint gate on this exact tree - fmt python/markdown, keep-sorted, ruff, mypy, feature
flags, pyscripts, test waits, changelog, patch/stitch terminology, symvision, toobig,
SASE validation, committed plans - and was then killed by a 15m IDLE timeout inside
run_silent-wrapped `just test-cost`, which emits nothing until it finishes. Run 2
(vccf65gycnky) died 41s in at `lint (feature flags)`. This run covers the only two
check-full gates that never produced a result: `just test-cost` and
`just selection-health --fail-on-new-flake`. The tree is unchanged since run 1 -
`git status` is still exactly pyproject.toml (sase-core-rs window 0.27.18 -> 0.29.0),
uv.lock, and one regex in tests/test_powerful_variables_landing.py - so run 1 lint
results still stand.

ALREADY-ADJUDICATED FAILURES. Do NOT re-diagnose these three; each is written up in the
close note already:

- `lint (feature flags)` rule 8, "live flag bead sase-qq has no definition (key
  plugin_catalog_scoped_latest)". NOT this epic. sase-qq was created
  2026-08-19T01:21:12Z by the still-running agent sase-qn.2 and its registry definition
  is on neither this HEAD nor origin/master. It is the known shared-store race, task
  sase-pp, which I corroborated with `sase bead +1 sase-pp` (now +2).
- tests/ace/tui/modals/test_project_inventory_subtabs.py::test_cross_navigation_and_escape_surface_disabled_workspaces -
  task sase-qo, root-caused and fixed by sase-qd.land at 2026-08-19T01:12:58Z but not
  yet committed to master; reproduces ~2 of 3 serial runs on master de06c55ca.
- tests/test_ace_testing.py::test_ace_page_fast_startup_is_structurally_quiet - open
  task sase-oz, +7.

IF this run is green, or red ONLY on those two test nodes, do exactly this, in order:

1. `sase bead epic-symbols sase-pv` - it reported "No --epic-symbol entries for sase-pv"
   minutes ago; if entries somehow appeared, resolve them per the Symvision
   epic-whitelist policy before closing.
2. Re-run `just _lint-flags` once. If it is green now, sase-qn.2 landed its definition
   in the meantime - say so in the line you append. If it still fails on sase-qq alone,
   that is the expected external red.
3. Append ONE line to /tmp/sase-pv-land/close_note.txt recording this run: this monitor
   id, the pytest pass/fail/skip counts, whether selection-health passed, and any node
   you excused. Then close:
   `sase bead close sase-pv --note "@/tmp/sase-pv-land/close_note.txt"`.
4. `just symvision` (or `just _lint-symvision`) to confirm the whitelist is clean.
5. Add `status: done` to the frontmatter of
   /home/bryan/.sase/plans/202608/flag_task_type.md (it has
   tier/title/goal/phases/proposed_by and no status field).
6. sase-pv has NO parent bead. Stop after the plan file; do not hunt for a parent phase
   or plan ancestor.

IF this run is red on ANYTHING ELSE, that is a real signal and almost certainly the
sase-core-rs 0.29.0 floor ratchet this landing carried (0.27.18 -> 0.29.0, applied with
tools/ratchet_core_window; 0.29.0 is what is installed in .venv). Diagnose it, fix the
cause, rerun through a monitor with no idle timeout, and close only once clean.

Then report to the user: what landed (the nine-phase flag-as-task-type epic), the
core-floor ratchet you carried and why it was required, and the three follow-ups
routed - sase-oz (+7), sase-qo, and sase-pp (+2). %xprompts_enabled:true
