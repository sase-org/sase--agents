- **AGENTS:**
  - [bbugyi200.athena.sase-qc--2](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-qc.md)

#fork:sase-qc--1 %model:sonnet %effort:high
...
**Why this was monitored:** Verify sase-qc occupancy guard fix with the scoped test
lane, since the toobig lint gate is red for a pre-existing, unrelated reason (tracked as
sase-q7)
...
Read the just test-scoped output. If it passed, run: sase bead close sase-qc --note
"Added ensure_workspace_not_occupied guard to prepare_opened_checkout
(preparation=runner) in src/sase/main/workspace_handler_list.py, matching the pattern in
run_agent_runner_setup.py/run_agent_exec_retry.py; added regression test
tests/main/test_workspace_open_clean_occupancy.py verifying
handle_open_clean/prepare_opened_checkout refuses when the checkout is occupied by a
live agent. Verified via just test-scoped (scoped test lane passed). Note: just check
separately fails at lint (toobig) on tests/_suite_gate.py (1197 lines, limit 1000) —
this is a pre-existing, unrelated failure already tracked as sase-q7 (I added a +1
corroboration); it does not touch this beads diff." Then report to the user: sase-qc is
closed, describe what was verified, and mention the pre-existing unrelated toobig
failure tracked as sase-q7. If test-scoped failed for a reason related to the occupancy
guard change (e.g. the new test in tests/main/test_workspace_open_clean_occupancy.py or
the guard wiring in workspace_handler_list.py), fix the issue, rerun just test-scoped
inline, and only close sase-qc once it passes. If it failed for a clearly unrelated
reason, report that to the user without closing the bead. %xprompts_enabled:true
