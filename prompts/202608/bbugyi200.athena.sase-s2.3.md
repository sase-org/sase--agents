- **AGENTS:**
  - [bbugyi200.athena.sase-s2.3--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-s2.3.md)

#fork:sase-s2.3--plan %model:grok-4.6 %effort:high
...
**Why this was monitored:** Prove sase-s2.3 combined approval-to-launch tree with
exhaustive checks
...
Continue bead sase-s2.3 only. The bead is already in_progress and assigned to you; do
not set status by hand. Do not close the parent epic sase-s2 or any ancestor.

Work already done in this workspace:

- Added tests/test_plan_approval_launch_reliability_integration.py (tale
  delayed-archive + coder artifact-link linear history; historical two-writer rebase
  conflict; inverted-scheduling archive publication; epic code-swap wait then one DAG).
- Stubbed host-owned archives in leftover commit-approval tests that had no project
  identity (auto-approval, action API, e2e smoke, TUI plan gate, telegram status
  override).
- Focused gate/archive/swap/monitor/epic suites passed; race tests passed three times
  with inverted scheduling. sase bead epic-symbols sase-s2.3 reported no leftovers.

If just check-full passed:

1. Run sase bead epic-symbols sase-s2.3 again. If any --epic-symbol entries remain,
   resolve each symbol or re-key the Justfile line to a still-open bead.
2. Close only this bead: sase bead close sase-s2.3 --note "<what you verified, including
   check-full>".
3. Finish with /sase_final. Do not make more file or repository changes after a
   successful sase final submit.

If just check-full failed:

- Fix failures caused by this phase (approval archive protocol, epic code-swap launch,
  or the new integration tests).
- tests/test_contract_manifest.py stale-manifest failures caused by a just-check
  sase-core fast-forward are out of scope: do not refresh the manifest unless this phase
  actually changed the marker set. Record PROPOSED FOLLOW-UP on sase-s2.3 instead.
- Do not create beads; use sase bead note sase-s2.3 'PROPOSED FOLLOW-UP: <one-line
  summary — detail>' for discovered follow-up.
- Re-run the failing tests, then just check if the failure was scoped, or start another
  just check-full monitor if the combined tree still needs it.
- Do not close sase-s2.3 until check-full is green or the remaining failures are
  documented as out-of-scope on the bead.

Do not create beads yourself. Do not close sase-s2. %xprompts_enabled:true
