- **AGENTS:**
  - [bbugyi200.athena.zv--4](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.zv.md)

#fork:zv--3 %model:sonnet %effort:xhigh
...
**Why this was monitored:** Re-verify diff-scoped test lane for monitor_duplicate_rows
plan after registering finalize_monitor_workflow_state and _has_monitored_done_marker
in the two marker-audit registries (the only two test-scoped failures last run)
...
Report just test-scoped results to the user. If it passed: the plan diff itself is fully
verified; tell the user that repo-wide just check/just check-full still cannot go green
because of a pre-existing, unrelated symvision blocker already owned by in-progress bead
sase-kz.8 (Justfile --epic-symbol entries reference closed bead sase-kz.5), and ask
whether to land anyway, wait for sase-kz.8, or take some other action -- do not attempt
to fix the symvision issue yourself. If it failed: report the failures; only fix root
causes that are part of the monitor_duplicate_rows plan diff
(src/sase/ace/tui/models/_loaders/,
src/sase/monitor/{reconcile,settlement,start,supervise}.py,
tests/test_agent_artifact_marker_mutation_audit.py,
tests/test_agent_artifact_marker_path_passing_audit.py, and their tests) and re-run just
test-scoped.
