- **PLAN:**
  [202608/stitch_origin_amend_footer.md](https://github.com/sase-org/sase--plans/blob/main/202608/stitch_origin_amend_footer.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-jo.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-jo.land.md)

You are the land agent for epic bead sase-jo: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-jo` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-jo, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   `sase bead close sase-jo --note "<what you verified in steps 1-2>"`. AFTER closing,
   run `just symvision` if available (epic-symbol whitelist entries for sase-jo expire
