- **AGENTS:**
  - [bbugyi200.athena.sase-p5.4--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-p5.4.md)

#fork:sase-p5.4--plan %model:sonnet %effort:xhigh
...
**Why this was monitored:** Verify sase-p5.4 shared-clone-exemption change before
...
Report pass/fail for sase-p5.4 (commit finalizer shared-clone race exemption). If just
check passed, run `sase bead epic-symbols sase-p5.4`, resolve any leftover epic-symbol
entries, then close the bead with
`sase bead close sase-p5.4 --note "<what you verified>"`. If it failed, show the failing
output and fix it, then re-run just check via another monitor. %xprompts_enabled:true
