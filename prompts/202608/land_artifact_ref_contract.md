- **PLAN:**
  [202608/land_artifact_ref_contract.md](https://github.com/sase-org/sase--plans/blob/main/202608/land_artifact_ref_contract.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-js.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-js.land.md)

You are the land agent for epic bead sase-js: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-js` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-js, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   `sase bead close sase-js --note "<what you verified in steps 1-2>"`. AFTER closing,
   run `just symvision` if available (epic-symbol whitelist entries for sase-js expire
