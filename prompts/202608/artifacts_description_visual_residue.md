- **PLAN:**
  [202608/artifacts_description_visual_residue.md](https://github.com/sase-org/sase--plans/blob/main/202608/artifacts_description_visual_residue.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-u6.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-u6.land.md)

You are the land agent for epic bead sase-u6: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-u6` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-u6, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-u6`. Every listed
   ...
   `sase bead close sase-u6 --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead show sase-u6`. If there is no parent bead, finish normally. If the parent is
