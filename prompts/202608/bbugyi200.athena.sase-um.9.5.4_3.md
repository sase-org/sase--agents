- **AGENTS:**
  - [bbugyi200.athena.sase-um.9.5.4--5](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-um.9.5.4.md)
...
**Why this was monitored:** Wait for Full CI 33244467241 on ca7692ee3 so ci_watch can
...
This monitor: Full CI 33244467241 (workflow_dispatch, in_progress) on origin/master tip
ca7692ee3. Master Gate 33244341435 was in progress on ca7692ee3. PR #284 OPEN
MERGEABLE/CLEAN (head ebf8dfe8). publish.yml 33244468516 (publish_existing=false) was in
progress. Cancelled obsolete Full CI 33240160035 on c1a5b36f5 so this run would not
queue behind it.

Landed this turn: ca7692ee3 after Full CI 33240119114 on c1a5b36f5 went RED. Failed
nodes: (1) full/test(3.12) job 99068560982 —
tests/test_running_agents_snapshot.py::test_list_all_agents_includes_done_and_failed
missed TS_ACE_RUN_RETRIED_CHILD (20260427140500, pid 33333) because pid_is_thread
treated a live host TID as the fixture process. test(3.14) and coverage-contexts green;
visual-test green. (2) full/contention-test job 99068561001 — runner shutdown/cancel
after the 3.12 failure, no harness assertion. just check was green (scoped 63 files).

Then:

1. If this monitor timed out while 33244467241 is still pending or in_progress, re-issue
   `sase monitor start` watching 33244467241 (or the newest Full CI on origin/master if
   that run finished or was superseded) with timeout at least 3h. Do not inline-wait
   Full CI.
2. If Full CI 33244467241 is red or cancelled, attribute failed nodes on the ca7692ee3
   run (ignore failures on c1a5b36f5, 49d6c4188, and 623788895). Fix in-scope,
...
3. If Full CI is green on ca7692ee3 (or the newest Full CI on the integrated
