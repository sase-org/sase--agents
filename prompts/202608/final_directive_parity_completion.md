- **PLAN:**
  [202608/final_directive_parity_completion.md](https://github.com/sase-org/sase--plans/blob/main/202608/final_directive_parity_completion.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-s0.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-s0.land.md)

You are the land agent for epic bead sase-s0: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-s0` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-s0, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-s0`. Every listed
   ...
   `sase bead close sase-s0 --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead show sase-s0`. If there is no parent bead, finish normally. If the parent is
