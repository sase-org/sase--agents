- **AGENTS:**
  - [bbugyi200.athena.0bz--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0bz.md)

#fork:0bz--code %model:grok-4.6 %effort:high
...
**Why this was monitored:** Scoped just check escalated (core-identity-changed); run
exhaustive verification after family-facing completion-notification identity projection
...
The approved plan 202608/agent_family_completion_notifications.md is already implemented
in this workspace: src/sase/axe/run_agent_runner_finalize.py now prefers agent_meta.json
agent_family for completion notes, action_data.agent_name, and bead-display lookup,
while keeping cl_name/raw_suffix for shell navigation. Tests live in
tests/test_run_agent_runner_notifications.py. just install and just check already
passed; the scoped lane escalated to the full suite (core-identity-changed) and that
escalated run passed. If just check-full passed, reply to the user that the plan is
implemented and summarize the identity projection. If it failed, fix the failures, rerun
verification as required, then reply. Do not redo the implementation unless check-full
shows a defect. %xprompts_enabled:true
