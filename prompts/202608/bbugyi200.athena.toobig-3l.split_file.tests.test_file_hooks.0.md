- **AGENTS:**
  - [bbugyi200.athena.toobig-3l.split_file.tests.test_file_hooks.0--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.toobig-3l.split_file.tests.test_file_hooks.0.md)

#fork:toobig-3l.split_file.tests.test_file_hooks.0--plan %model:grok-4.6 %effort:xhigh
...
If just check passed, use /sase_final and reply to the user: tests/test_file_hooks.py
(726 lines) was split by concern into tests/_file_hooks_helpers.py (shared fixtures),
tests/test_file_hooks_loader.py (config loading/validation), and
tests/test_file_hooks_matching.py (event matching). All files are well under 500 lines;
all 30 original tests (40 pytest cases) were preserved and already passed locally. If
just check failed, fix the reported issues, re-run just check if remaining work is short
(or /sase_monitor again if long), then reply. Do not change the split unless a gate
requires it. %xprompts_enabled:true
