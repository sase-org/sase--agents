- **PLAN:**
  [202608/docs_pdf_test_pypdf_import_skip.md](https://github.com/sase-org/sase--plans/blob/main/202608/docs_pdf_test_pypdf_import_skip.md)
- **AGENTS:**
...
Can you complete the work for bead sase-m4.6? The bead is already reserved for you and
...
`sase bead close sase-m4.6 --note "<what you verified>"`. Closing an assigned phase bead
...
via `sase bead note sase-m4.6 'PROPOSED FOLLOW-UP: <one-line summary — detail>'`; the
...
This agent delegated the remaining work to a monitor member. Monitor ID: scqjm9yddr5v
Inspect with: sase monitor show scqjm9yddr5v Monitor member: sase-m4.6--mon Directory:
...
sase-m4, and do not create task beads. If just check-full failed or timed out, inspect
the retained log, fix any attributable failure in this workspace, rerun focused checks
plus just check as appropriate, then start another monitored just check-full with this
same continuation. If a failure is demonstrably unrelated/pre-existing, record it on
sase-m4.6 with: sase bead note sase-m4.6 "PROPOSED FOLLOW-UP: <one-line summary —
detail>". If just check-full passed, verify post-landing GitHub Actions for the exact
landed commit e4baf07717f5a9cb836316b8db5416d1af3f8096 on repo sase-org/sase: find the
workflow run(s) for that head SHA, do not monitor an older run, and if any relevant run
is pending use sase monitor start around gh run watch <exact-run-id> --repo
sase-org/sase --exit-status with a next action to resume this same verification. Run
actstat and confirm the latest sase project GitHub Actions run is passing. Verify every
workflow triggered for e4baf07717f5a9cb836316b8db5416d1af3f8096, including CI, Docs, and
Publish where applicable, is terminal and successful rather than
queued/stalled/failed/cancelled. If Actions are not stable green, inspect the exact new
job logs and create/propose a repair plan with /sase_plan before making further file
changes. Once local just check-full and exact-commit Actions are green, close only the
phase bead with: sase bead close sase-m4.6 --note "Verified just install; focused
ratchet/docs/TUI/finalizer pytest; ratchet --check; uv lock --check; Python 3.12
release-core-floor smoke; just docs-check; just docs-pdf-check; targeted agent-scan
benchmark; just phase7-perf-check; just test-visual; just check; monitored just
check-full; and exact-commit GitHub Actions/actstat for
e4baf07717f5a9cb836316b8db5416d1af3f8096." Then reply to the user with the verification
summary.
...
**Why this was monitored:** Run the required exhaustive integration gate for phase bead
sase-m4.6
...
This agent delegated the remaining work to a monitor member. Monitor ID: 1rhbn6t96r3q
Inspect with: sase monitor show 1rhbn6t96r3q Monitor member: sase-m4.6--mon-0 Directory:
/home/bryan/.local/state/sase/workspaces/sase-org/sase/sase_10
...
Verify post-landing CI run for phase bead sase-m4.6 exact commit
e4baf07717f5a9cb836316b8db5416d1af3f8096; Docs and Publish already succeeded, CI still
in_progress

Next action:

Continue phase bead sase-m4.6 verification. Run `actstat` and confirm the sase project's
latest GitHub Actions run is passing; confirm CI run 31832121634 for commit
e4baf07717f5a9cb836316b8db5416d1af3f8096 concluded successfully (not
failed/cancelled/timed out). Do not set bead status by hand, do not close parent epic
sase-m4, and do not create task beads directly (use
`sase bead note sase-m4.6 "PROPOSED FOLLOW-UP: ..."` for discovered issues instead). If
CI failed, inspect the exact new job logs and create/propose a repair plan with
/sase_plan before making further file changes. If CI passed and actstat confirms the
latest run is green, close only the phase bead with: sase bead close sase-m4.6 --note
"Verified just install; focused ratchet/docs/TUI/finalizer pytest; ratchet --check; uv
lock --check; Python 3.12 release-core-floor smoke; just docs-check; just
docs-pdf-check; targeted agent-scan benchmark; just phase7-perf-check; just test-visual;
just check; monitored just check-full (full pytest suite passed cleanly; only the
pre-existing/unrelated flake-baseline landing gate tripped on historical cross-workspace
flake evidence unrelated to this phase, documented as a PROPOSED FOLLOW-UP note rather
than masked); and exact-commit GitHub Actions/actstat green for
e4baf07717f5a9cb836316b8db5416d1af3f8096 (CI, Deploy Docs, Publish all successful)."
Then reply to the user with the verification summary.
...
**Why this was monitored:** Verify post-landing CI run for phase bead sase-m4.6 exact
commit e4baf07717f5a9cb836316b8db5416d1af3f8096; Docs and Publish already succeeded, CI
still in_progress
...
than masked); and exact-commit GitHub Actions/actstat green for
e4baf07717f5a9cb836316b8db5416d1af3f8096 (CI, Deploy Docs, Publish all successful)."
Then reply to the user with the verification summary. %xprompts_enabled:true
