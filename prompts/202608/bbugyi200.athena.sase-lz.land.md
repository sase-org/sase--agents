- **AGENTS:**
  - [bbugyi200.athena.sase-lz.land--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-lz.land.md)

#fork:sase-lz.land--code %model:gpt-5.5 %effort:high
...
**Why this was monitored:** Run final full verification for approved sase-lz landing
after focused tests and just check passed
...
Continue the approved sase-lz landing workflow. Inspect the just check-full result,
distinguish unrelated failures from epic regressions, and fix any epic-caused failures.
Context: focused Models-panel tests passed (128 tests); just test-visual failed only
tests/ace/tui/visual/test_ace_png_snapshots_agents_retry_e2e.py::test_real_fakey_running_fallback_png_snapshot,
unrelated to the selector-builder diff, and was recorded as a verified-after-close +1 on
duplicate task sase-dc with artifact refs; just check passed. Current intended repo diff
is docs/ace.md, src/sase/ace/tui/modals/models_panel_selector_builder.py, and
tests/test_models_panel_selector_builder.py. After full verification, review the diff,
commit through /sase_git_commit, close bead sase-lz with the plan-required summary, run
just symvision and clean up/commit any stale sase-lz symbols if reported, then mark
plan:202608/models_panel_pool_authoring.md frontmatter status done and confirm sase-lz
is CLOSED. %xprompts_enabled:true
