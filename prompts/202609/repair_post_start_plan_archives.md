- **PLAN:**
  [202609/repair_post_start_plan_archives.md](https://github.com/sase-org/sase--plans/blob/main/202609/repair_post_start_plan_archives.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-z2.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-z2.land.md)

You are the land agent for epic bead sase-z2: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-z2` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-z2, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-z2`. Every listed
   ...
   `sase bead close sase-z2 --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead show sase-z2`. If there is no parent bead, finish normally. If the parent is
