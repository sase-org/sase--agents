- **AGENTS:**
  - [bbugyi200.athena.toobig-5p.commit_dispatch_followup.0--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.toobig-5p.commit_dispatch_followup.0.md)

%queue(weight=1) %auto #fork:toobig-5p.commit_dispatch_followup.0--plan
...
**Why this was monitored:** Verify the commit-dispatch follow-up module split before
...
Inspect the just check result. If it passed, review the final diff and line counts, then
submit the required SASE final declaration and reply concisely. If it failed because of
the split, fix it, run just fix and just check as required, then finalize.
