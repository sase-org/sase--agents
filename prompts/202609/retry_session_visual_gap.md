- **PLAN:**
  [202609/retry_session_visual_gap.md](https://github.com/sase-org/sase--plans/blob/main/202609/retry_session_visual_gap.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-17m.5.1.6.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-17m.5.1.6.land.md)

You are the land agent for epic bead sase-17m.5.1.6: verify the epic is truly complete,
...
1. Verify. Run
   `sase bead read sase-17m.5.1.6 -r "Need the epic scope, children, and linked plan file"`,
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-17m.5.1.6, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-17m.5.1.6`. Every listed
   ...
   `sase bead close sase-17m.5.1.6 --note "<what you verified in steps 1-2>"`. After
   ...
   shown by `sase bead read`). If the close is rejected because leftover `--epic-symbol`
...
`sase bead read sase-17m.5.1.6 -r "Need the parent link"`. If there is no parent bead,
finish normally. If the parent is a phase bead, verify this child plan completed the
work required by that phase, close only that parent phase normally with
