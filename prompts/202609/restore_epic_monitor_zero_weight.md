- **PLAN:**
  [202609/restore_epic_monitor_zero_weight.md](https://github.com/sase-org/sase--plans/blob/main/202609/restore_epic_monitor_zero_weight.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-10h.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-10h.land.md)

You are the land agent for epic bead sase-10h: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-10h` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-10h, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-10h`. Every listed
   ...
   `sase bead close sase-10h --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead show sase-10h`. If there is no parent bead, finish normally. If the parent is
