- **PLAN:**
  [202609/finish_pager_target_ownership.md](https://github.com/sase-org/sase--plans/blob/main/202609/finish_pager_target_ownership.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-xy.5.5.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-xy.5.5.land.md)

You are the land agent for epic bead sase-xy.5.5: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-xy.5.5` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-xy.5.5, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-xy.5.5`. Every listed
   ...
   `sase bead close sase-xy.5.5 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-xy.5.5`. If there is no parent bead, finish normally. If the parent
