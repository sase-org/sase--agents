- **AGENTS:**
  - [bbugyi200.athena.research.28.final](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.research.28.final/README.md)

%clan(research.28, tribe=research, summary=[[[bold]RESEARCH PROMPT:[/bold] We recently
implemented the `sase tool` command, which is just the first part of a larger plan (see
the sase_tool_epic_roadmap.md file in the research sidecar repo and the sase-135 epic
bead for context). I'm thinking about taking the next steps on this. Here's what I'm
thinking that those next steps should be:
...
%id:research.28.final %m:@xlarge %wait:research.28.cld %wait:research.28.mus
%wait:research.28.gem %q(w=0.25) #gh:gh_sase-org__sase You are the lead researcher: 3
...
1. From the registered reports above, identify exactly one report per expected suffix in
   cld, mus, gem, belonging to this dispatch's `research.28.cld`, `research.28.mus`,
   `research.28.gem` dependencies, matching by `wait_name` and the canonical research
