- **AGENTS:**
  - [bbugyi200.athena.research.1z.cdx](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.research.1z.cdx/README.md)

%clan(research.1z, tribe=research, summary=[[[bold]RESEARCH PROMPT:[/bold] I want to add
a new `%sink` directive that acts like the `%wait` directive, but in reverse (i.e. the
target agents wait for the agent that specified the `%sink` directive in its prompt).
...
%id:research.1z.cdx %m:@sol_or_grok %q(w=0.25) #gh:gh_sase-org**sase You are researcher
A in a two-researcher swarm. The other researcher, `research.1z.cld`, is independently
...
I want to add a new `%sink` directive that acts like the `%wait` directive, but in
reverse (i.e. the target agents wait for the agent that specified the `%sink` directive
in its prompt).
