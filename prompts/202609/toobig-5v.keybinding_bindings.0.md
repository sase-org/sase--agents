- **AGENTS:**
  - [bbugyi200.athena.toobig-5v.keybinding_bindings.0--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.toobig-5v.keybinding_bindings.0.md)

%queue(weight=1) %auto #fork:toobig-5v.keybinding_bindings.0--plan
...
**Why this was monitored:** Verify the _keybinding_bindings.py split (facade + 4
submodules) before replying
...
The split of src/sase/ace/tui/widgets/_keybinding_bindings.py is implemented: a 43-line
facade plus _keybinding_bindings_chips.py, _keybinding_bindings_axe.py,
_keybinding_bindings_patch.py, and _keybinding_bindings_agents.py (all <=500 lines;
method bodies moved verbatim, proven by AST comparison; ruff and smoke test already
passed). If the check passed, submit the sase_final commit declaration for the main repo
with a Conventional Commit message describing the split, then reply to the user with a
short summary and the verification evidence. If the check failed, fix the failures,
re-run verification, then submit and reply. %xprompts_enabled:true
