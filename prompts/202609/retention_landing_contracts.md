- **PLAN:**
  [202609/retention_landing_contracts.md](https://github.com/sase-org/sase--plans/blob/main/202609/retention_landing_contracts.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-zw.8.7.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-zw.8.7.land.md)

You are the land agent for epic bead sase-zw.8.7: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-zw.8.7` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-zw.8.7, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-zw.8.7`. Every listed
   ...
   `sase bead close sase-zw.8.7 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-zw.8.7`. If there is no parent bead, finish normally. If the parent
