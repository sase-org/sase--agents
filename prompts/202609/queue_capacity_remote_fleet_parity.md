- **PLAN:**
  [202609/queue_capacity_remote_fleet_parity.md](https://github.com/sase-org/sase--plans/blob/main/202609/queue_capacity_remote_fleet_parity.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-zt.6.5.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-zt.6.5.land.md)

You are the land agent for epic bead sase-zt.6.5: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-zt.6.5` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-zt.6.5, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-zt.6.5`. Every listed
   ...
   `sase bead close sase-zt.6.5 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-zt.6.5`. If there is no parent bead, finish normally. If the parent
