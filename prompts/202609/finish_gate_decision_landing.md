- **PLAN:**
  [202609/finish_gate_decision_landing.md](https://github.com/sase-org/sase--plans/blob/main/202609/finish_gate_decision_landing.md)
- **AGENTS:**
  - [bbugyi200.apollo.sase-zr.7.1.1.5.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.apollo.sase-zr.7.1.1.5.land.md)

You are the land agent for epic bead sase-zr.7.1.1.5: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-zr.7.1.1.5` (children, linked plan file), review the
   epic bead's own notes, then run `sase bead show` on every child and review every
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-zr.7.1.1.5, excluding the epic's own commits; in a PR workflow
   also review commits on the base branch) and update anything that should now use what
   this epic added or that duplicates or conflicts with it. This integration is part of
   the epic's work.
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-zr.7.1.1.5`. Every
   ...
   the epic with
   `sase bead close sase-zr.7.1.1.5 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-zr.7.1.1.5`. If there is no parent bead, finish normally. If the
