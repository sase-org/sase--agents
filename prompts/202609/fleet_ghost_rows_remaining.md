...
- **AGENTS:**
  - [bbugyi200.apollo.sase-xe.16.11.7.16.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.apollo.sase-xe.16.11.7.16.land.md)

You are the land agent for epic bead sase-xe.16.11.7.16: verify the epic is truly
complete, integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-xe.16.11.7.16` (children, linked plan file), review
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-xe.16.11.7.16, excluding the epic's own commits; in a PR
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-xe.16.11.7.16`. Every
   ...
   the epic with
   `sase bead close sase-xe.16.11.7.16 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-xe.16.11.7.16`. If there is no parent bead, finish normally. If the
