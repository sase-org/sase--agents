- **AGENTS:**
  - [bbugyi200.athena.toobig-52.artifact_link_publication_retry.0](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.athena.toobig-52.artifact_link_publication_retry.0/README.md)

%wait:toobig-52.machine_init.0 %id(artifact_link_publication_retry.0, clan=toobig-52)
%model:@medium %auto %queue(runners=3, priority=20) #gh:gh_sase-org__sase Can you help
me split the `src/sase/sdd/_artifact_link_publication_retry.py` file up into multiple
files? Use your best judgement, but let's aim to keep all files <=500 lines of code.
