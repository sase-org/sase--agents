- **AGENTS:**
  - [bbugyi200.athena.sase-x7.2.1.3--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-x7.2.1.3.md)

#fork:sase-x7.2.1.3 %model:@small
...
**Why this was monitored:** Run exhaustive verification for bead sase-x7.2.1.3 after
migration kit driver work
...
is already in the working tree. Focused tests passed:
`uv run pytest tests/migration_kit tests/main/test_migrate_parser.py tests/main/test_parser_command_defaults.py tests/main/test_migrate_startup_isolation.py tests/test_check_sase_core_rs_bindings_tool.py -q`;
exact prior full-suite flake tests passed when rerun directly; and a PROPOSED FOLLOW-UP
note was added for those flakes. Inspect the monitor outcome for `just check-full`. If
it failed only on unrelated tests that pass when rerun directly, record that evidence on
the bead as another PROPOSED FOLLOW-UP note rather than changing unrelated code. Then
run `sase bead epic-symbols sase-x7.2.1.3`, resolve or re-key any leftover symbols,
close only `sase-x7.2.1.3` with
`sase bead close sase-x7.2.1.3 --note "<what was verified>"`, run the SASE final
declaration skill, and send a concise final response. Do not set bead status by hand and
do not close any parent or ancestor bead. %xprompts_enabled:true
