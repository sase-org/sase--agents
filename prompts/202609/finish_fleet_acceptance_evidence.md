- **PLAN:**
  [202609/finish_fleet_acceptance_evidence.md](https://github.com/sase-org/sase--plans/blob/main/202609/finish_fleet_acceptance_evidence.md)
- **AGENTS:**
  - [bbugyi200.apollo.sase-xe.16.11.7.16.5.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.apollo.sase-xe.16.11.7.16.5.land.md)

You are the land agent for epic bead sase-xe.16.11.7.16.5: verify the epic is truly
complete, integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-xe.16.11.7.16.5` (children, linked plan file),
   review the epic bead's own notes, then run `sase bead show` on every child and review
   every child note. Confirm each note was addressed, and read the actual source code
   and the epic's commits (bead IDs appear in commit messages) to confirm the work
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-xe.16.11.7.16.5, excluding the epic's own commits; in a PR
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-xe.16.11.7.16.5`. Every
   ...
   the epic with
   `sase bead close sase-xe.16.11.7.16.5 --note "<what you verified in steps 1-2>"`.
   After closing, run `just symvision` if available to confirm the whitelist is clean.
...
`sase bead show sase-xe.16.11.7.16.5`. If there is no parent bead, finish normally. If
the parent is a phase bead, verify this child plan completed the work required by that
