- **PLAN:**
  [202609/queue_multiplier_loaded_display.md](https://github.com/sase-org/sase--plans/blob/main/202609/queue_multiplier_loaded_display.md)
- **AGENTS:**
  - [bbugyi200.apollo.sase-19f.6.land--plan](https://github.com/sase-org/sase--agents/blob/main/sessions/bbugyi200.apollo.sase-19f.6.land.md)

You are the land agent for epic bead sase-19f.6: verify the epic is truly complete,
...
1. Verify. Run
   `sase bead read sase-19f.6 -r "Need the epic scope, children, and linked plan file"`,
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-19f.6, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-19f.6`. Every listed
   ...
   `sase bead close sase-19f.6 --note "<what you verified in steps 1-2>"`. After
...
`sase bead read sase-19f.6 -r "Need the parent link"`. If there is no parent bead,
