- **PLAN:**
  [202609/remote_send_keys_hint_quoting.md](https://github.com/sase-org/sase--plans/blob/main/202609/remote_send_keys_hint_quoting.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-123.7.6.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-123.7.6.land.md)

You are the land agent for epic bead sase-123.7.6: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-123.7.6` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-123.7.6, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-123.7.6`. Every listed
   ...
   `sase bead close sase-123.7.6 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-123.7.6`. If there is no parent bead, finish normally. If the
