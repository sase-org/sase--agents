- **AGENTS:**
  - [bbugyi200.athena.0rp--2](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.0rp.md)

%queue(weight=1) #fork:0rp--1 %model:@small
...
**Why this was monitored:** Run the required whole-repository check for the approved
toobig gate change
...
Inspect the check result. If it passed, inspect the final diff and submit the required
SASE final declaration with a conventional commit message. If it failed, fix only
failures caused by the approved toobig gate change, run just fix and the focused tests,
then rerun verification through a monitor. %xprompts_enabled:true
