- **AGENTS:**
  - [bbugyi200.apollo.research.3.cdx](https://github.com/sase-org/sase--agents/blob/main/agents/bbugyi200.apollo.research.3.cdx/README.md)

%clan(research.3, tribe=research, summary=[[[bold]RESEARCH PROMPT:[/bold] I want to plan
the work required to add a new `sase tool` command soon, but am not sure where to start.
...
recommended solution / set of epics.]]) %id:research.3.cdx %m:@sol_or_grok %q(w=0.25)
#gh:gh_sase-org**sase You are researcher A in a two-researcher swarm. The other
researcher, `research.3.cld`, is independently investigating the same request and will
...
I want to plan the work required to add a new `sase tool` command soon, but am not sure
where to start.
...
work up? Is this work really large enough that we can split it up into multiple epics,
each with verifiable (from a normal user) results? If so, end your analysis with a
recommended solution / set of epics. #research(suffix=a)
