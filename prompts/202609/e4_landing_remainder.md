...
- **AGENTS:**
  - [bbugyi200.athena.sase-1ah.land--plan](https://github.com/sase-org/sase--agents/blob/main/sessions/bbugyi200.athena.sase-1ah.land.md)

You are the land agent for epic bead sase-1ah: verify the epic is truly complete,
...
1. Verify. Run
   `sase bead read sase-1ah -r "Need the epic scope, children, and linked plan file"`,
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-1ah, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-1ah`. Every listed
   ...
   `sase bead close sase-1ah --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead read sase-1ah -r "Need the parent link"`. If there is no parent bead, finish
