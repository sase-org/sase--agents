- **PLAN:**
  [202609/finish_double_star_model_completion.md](https://github.com/sase-org/sase--plans/blob/main/202609/finish_double_star_model_completion.md)
- **AGENTS:**
  - [bbugyi200.athena.sase-yw.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-yw.land.md)

You are the land agent for epic bead sase-yw: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-yw` (children, linked plan file), review the epic
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-yw, excluding the epic's own commits; in a PR workflow also
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-yw`. Every listed
   ...
   `sase bead close sase-yw --note "<what you verified in steps 1-2>"`. After closing,
...
`sase bead show sase-yw`. If there is no parent bead, finish normally. If the parent is
