- **AGENTS:**
  - [bbugyi200.athena.sase-xe.land--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-xe.land.md)

#fork:sase-xe.land %model:claude-fable-5 %effort:xhigh
...
**Why this was monitored:** Landing gate for the epic sase-xe integration tree
(recreated xe.7 tests, provider fixes, epic-symbol cleanup, core-side flag removal)
...
You are continuing the sase-xe landing (full context is in this family transcript and
the LANDING PROGRESS note on bead sase-xe). just check-full was the landing gate for the
uncommitted integration tree in this workspace plus its linked sase-core checkout. 1)
Evaluate the results. Known master-side noise to triage against open beads rather than
this tree: sase-j0 (suite-cost budgets exceeded), sase-x5 (29 PNG golden drift on clean
master), sase-x4 (silent test-cost lease hang), and flake beads such as sase-xb,
sase-vl, sase-x1, sase-x2, sase-wh, sase-wv, sase-xa. Confirm suspected flakes in
isolation and corroborate via sase bead +1 with evidence; fix anything genuinely caused
by this tree and re-verify the affected slice. 2) Then end the turn normally with your
/sase_final skill, declaring a commit decision for BOTH dirty repositories: the primary
sase workspace (suggested message: "feat(dispatch): integrate and harden remote dispatch
during the sase-xe landing") and the linked sase-core checkout (suggested message:
"feat(editor): drop the removed remote_dispatch directive gate (sase-xe)"). Do NOT
invoke /sase_git_commit, and do NOT propose the child plan this turn - a plan-propose
handoff skips the commit finalizers and would strand this tree (see the landing
note). 3) In your final response tell the user: the sase-xe landing is intentionally
paused mid-flight; once these commits land, rerun "sase bead work sase-xe" so a fresh
land agent proposes the prepared child epic plan (stashed at
/home/bryan/.sase/projects/gh_sase-org__sase/artifacts/ace-run/202609/07/20260907172341/sase_plan_remote_dispatch_acceptance_hardening.md
and fully described in the sase-xe landing note) and then finishes the close.
