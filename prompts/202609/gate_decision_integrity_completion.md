...
- **AGENTS:**
  - [bbugyi200.apollo.sase-zr.7.1.1.land--plan](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.apollo.sase-zr.7.1.1.land.md)

You are the land agent for epic bead sase-zr.7.1.1: verify the epic is truly complete,
integrate it with changes that landed since it started, then close it out.

1. Verify. Run `sase bead show sase-zr.7.1.1` (children, linked plan file), review the
...
2. Integrate. Changes committed since this epic started could not integrate with this
   ...
   commit mentioning sase-zr.7.1.1, excluding the epic's own commits; in a PR workflow
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them
   ...
   close note. Before closing, run `sase bead epic-symbols sase-zr.7.1.1`. Every listed
   ...
   `sase bead close sase-zr.7.1.1 --note "<what you verified in steps 1-2>"`. After
...
`sase bead show sase-zr.7.1.1`. If there is no parent bead, finish normally. If the
