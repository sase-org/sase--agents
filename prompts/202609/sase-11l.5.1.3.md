- **AGENTS:**
  - [bbugyi200.athena.sase-11l.5.1.3--1](https://github.com/sase-org/sase--agents/blob/main/families/bbugyi200.athena.sase-11l.5.1.3.md)

%queue(weight=1) %auto #fork:sase-11l.5.1.3--plan %model:sonnet@xhigh
...
**Why this was monitored:** Verify the preview-confirm phase (sase-11l.5.1.3): full
lint + test suite before closing the bead
...
just check-full has finished for bead sase-11l.5.1.3 (preview-confirm phase of the %hold
directive epic). Review the check-full result. If it failed, diagnose and fix the
reported issue(s), then rerun just check (inline is fine for a quick recheck; use
another monitor only if check-full itself needs to rerun). Once green: (1) run
`sase bead epic-symbols sase-11l.5.1.3` and resolve any --epic-symbol leftovers by
re-keying the Justfile line to a still-open bead (the parent epic sase-11l.5.1 or a
later phase) -- this phase should not have introduced any epic-symbol entries, so an
empty result is expected but must be confirmed; (2) if the sase-core-rs floor-pin
staleness warning from core-floor-probe (sase-core-rs missing
collect_hold_fields/format_hold_directive/hold_fields_to_selectors/decide_gate_lifecycle
capabilities relative to a published release) still appears and looks actionable, record
it with
`sase bead note sase-11l.5.1.3'PROPOSED FOLLOW-UP: sase-core-rs floor pin is stale relative to published sase-core capabilities already used by this epic; bump per tools/ratchet_core_revision.'`
-- do NOT create a bead yourself; (3) close ONLY sase-11l.5.1.3 with
`sase bead close sase-11l.5.1.3 --note "<what you verified>"`. Never close sase-11l.5.1
(the parent epic) or sase-11l.5 or sase-11l. Then run /sase_final.
