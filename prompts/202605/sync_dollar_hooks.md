- **PLAN:** [202605/sync_dollar_hooks.md](https://github.com/sase-org/sase--plans/blob/main/202605/sync_dollar_hooks.md)

 Can you help me make sure that whenever the sync operation associated with the `Y` keymap on the "CLs" tab of
the `sase ace` TUI completes, that all HOOKS entries that use the special `$` prefix have the status line associated
with the latest COMMITS entry removed? The goal is to trigger these hooks to re-run on the latest commit after syncing.
Think this through thoroughly and create a plan using your `/sase_plan` skill before making any file changes.
