...
as well that is configured to load-balance between the `claude/sonnet` and
`codex/gpt-5.5` models?

- We will need to add support for model alias load-balancing for this new model
  alias, which should have a value of `claude/sonnet | codex/gpt-5.5` (we will
...
- We should also be able to use this model alias load-balancing syntax to ensure
  ...
  model alias should always render to `claude/sonnet`).
