...
**Why this was monitored:** Full-suite landing gate for epic sase-sq: the scoped selector escalates on this landing diff, and just check-full aborts at lint (symvision) on the unrelated sase-tb blocker, so the full test suite is the remaining verification before the epic closes
