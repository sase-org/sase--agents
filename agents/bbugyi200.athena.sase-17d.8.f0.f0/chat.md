# Chat History - ace-run (sase-17d.8.f0.f0)

- **TIMESTAMP:** 2026-09-24 10:16:40 EDT
- **MODEL:** claude/opus
- **AGENT:** sase-17d.8.f0.f0
...
## Continuation Block `block:v1:0aa67bc73c575fc862262e015a770798`

- **Node:** `agent-delta:20260924095939:2c55ae074067c352`
- **Kind:** `agent_delta`
- **Parents:** (none)
- **Content:** `local:continuation/records/agent_delta/agent-delta:20260924095939:2c55ae074067c352.json`
...
# New Query
 ...
 Add a good note to the sase-17d epic bead for the epic lander agent instructing it to complete this unfinished work (no task beads for unfinished work that is in-scope for this epic).

## Response

Found a real issue while gathering evidence: `panel.py` is now 1051 lines, which is over the 1000-line toobig limit. I'm confirming this before I write the note.

Confirmed: master fails toobig, and no spread goldens exist. Next I'll write the note to the epic bead.

I added a note (#2) to the `sase-17d` epic bead for its land agent. It says the unfinished verification from `sase-17d.8` is in scope for the epic and must be completed before `sase-17d` closes. It also says no task beads should be filed for it, including for `.8`'s PROPOSED FOLLOW-UP notes #1–#3, which describe the same work. Each item points back to the relevant section of `deck_spread_mode.md` (§9, §10, §11):

1. **File too big:** `src/sase/ace/tui/widgets/decks/panel.py` must be split so it's back under the 1000-line limit. The note says to move the remaining spread and Files-probe logic into `panel_spread.py`, and to first check whether `sase-17d.10` has already made the file smaller.
2. **Golden screenshots:** add three reference screenshots (spread Main, spread Files, and a Main deck that switches to one card per page), then inspect every image.
3. **Live screenshots:** capture single spread Main, a split with one spread and one paged panel, and spread Files. Check the separator styling and that the title pill follows the card being read.
4. **Speed benchmark:** run `bench_tui_jk` with the decks flag on, in single-panel and side-by-side layouts, before and after `329d4049b`. p95 must stay under 16 ms. Find out why `.8` had 6 benchmark failures, and put both numbers in the epic close note.
5. **Full check:** `just check` must pass cleanly. Anything unrelated that is still failing gets reported with evidence rather than described as verified.

It ends with an explicit list of what must be true before the epic can close.

**Master is failing `just check` right now.** While gathering evidence I found that `panel.py` is 1051 lines on master (`12b253c35`), and the `toobig` check fails on it. It was 972 lines when `.8` finished, but when it was rebased onto `.9`'s layout-persistence change it went over 1000. Every agent running `just check` will hit this failure until the epic lander gets to it. If you don't want to wait that long, I can do the split in a separate small fix now.

No repository files changed, so there was nothing to commit.
