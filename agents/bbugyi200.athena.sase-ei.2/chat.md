...
%w:sase-ei.1
%w(bead=sase-ei.1)
[#bd/work_phase_bead:sase-ei.2](https://github.com/sase-org/sase/blob/3ac2b097beac842dc02df1edf88704ff87cd351d/src/sase/default_config.yml#L1002)
...
```markdown
Can you complete the work for bead sase-ei.2? The bead is already reserved for
...
description and design file, do the work, and close the bead with `sase bead close sase-ei.2 --note "<what you verified>"`. Closing
...
follow-up work as a `PROPOSED FOLLOW-UP:` entry via `sase bead note sase-ei.2 'PROPOSED FOLLOW-UP: <one-line summary — detail>'`; the epic's land agent triages these
into task beads.
```
