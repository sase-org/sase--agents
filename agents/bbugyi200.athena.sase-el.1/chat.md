...
```markdown
Can you complete the work for bead sase-el.1? The bead is already reserved for
...
description and design file, do the work, and close the bead with `sase bead close sase-el.1 --note "<what you verified>"`. Closing
...
follow-up work as a `PROPOSED FOLLOW-UP:` entry via `sase bead note sase-el.1 'PROPOSED FOLLOW-UP: <one-line summary — detail>'`; the epic's land agent triages these
into task beads.
```
