...
**Why this was monitored:** Wait for Full CI 33249788999 on 4a8b8358f so ci_watch can leave heavy_workflow_not_green
...
This monitor: Full CI 33249788999 (workflow_dispatch) on origin/master tip 4a8b8358f. Master Gate 33249652244 was in progress on 4a8b8358f. PR #284 OPEN MERGEABLE/CLEAN (head 7938caa41). publish.yml 33249789774 (publish_existing=false) was queued on 4a8b8358f.

Landed this turn: 4a8b8358f after Full CI 33244467241 on ca7692ee3 went RED. Failed nodes: (1) full/visual-test job 99080268574 — test_config_center_flags_narrow_png_snapshot PNG mismatch config_center_flags_populated_70x32 (9499 px); title clipped before SUNSET wrapped. (2) full/coverage-contexts job 99080268747 — test_updates_pane_sase_update_confirm_executes_and_refreshes wait_for 5s; set_timer mock swallowed non-1.0 timers. test(3.12) green; test(3.13)/test(3.14) cancelled after ~90m, no harness assertion. Master Gate 33244341435 also red: test(6) mini-xprompt verdict still Checking after pause(0.25); test(7) FrontmatterPanel on_mount NoMatches after blocked Path.read_text (production race not patched). just check green (scoped 65 files).

Then:
1. If this monitor timed out while 33249788999 is still pending or in_progress, re-issue `sase monitor start` watching 33249788999 (or the newest Full CI on origin/master if that run finished or was superseded) with timeout at least 3h. Do not inline-wait Full CI.
2. If Full CI 33249788999 is red or cancelled, attribute failed nodes on the 4a8b8358f run (ignore failures on ca7692ee3, c1a5b36f5, 49d6c4188, and 623788895). Fix in-scope, `just check`, land with `sase_git_commit -B`, redispatch Full CI and publish.yml (publish_existing=false), confirm PR #284 is still MERGEABLE/CLEAN (re-dispatch publish.yml without publish_existing if CONFLICTING/DIRTY), and monitor the new Full CI (3h+). Do not mute flakes; record PROPOSED FOLLOW-UP.
3. If Full CI is green on 4a8b8358f (or the newest Full CI on the integrated origin/master tip), dry-run ci_watch again. If still heavy_workflow_not_green, confirm the green Full CI SHA matches origin/master. Then watch live five-minute ci_watch ticks until sase-org/sase is eligible. The live `gh pr merge --merge --match-head-commit` is the acceptance evidence. Never hand-merge. Stay inside the 6-hour heavy window.
