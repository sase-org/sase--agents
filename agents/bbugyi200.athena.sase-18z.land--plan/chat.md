# Chat History - ace-run (sase-18z.land--plan)

- **TIMESTAMP:** 2026-09-25 10:45:22 EDT
- **MODEL:** codex/gpt-6-sol
- **AGENT:** sase-18z.land--plan

**Plan:** /home/bryan/.sase/plans/202609/bead_note_split_layout.md


## Prompt

#gh:gh_sase-org__sase
%id(land, clan=sase-18z, bead=sase-18z)
%model:@large
%auto
%w:sase-18z.1,sase-18z.2
%w(bead=sase-18z.1)
%w(bead=sase-18z.2)
You are the land agent for epic bead sase-18z: verify the epic is truly complete, integrate it with changes
...
1. Verify. Run `sase bead read sase-18z -r "Need the epic scope, children, and linked plan file"`, review the epic bead's own notes, then
...
2. Integrate. Changes committed since this epic started could not integrate with this epic's feature while it was
   incomplete. Find them (e.g. `git log` since the first commit mentioning sase-18z, excluding the epic's own
...
3. Land. Unresolved issues caused by this epic remain epic work: plan and finish them before closing. For each
   ...
   closing, run `sase bead epic-symbols sase-18z`. Every listed `--epic-symbol` entry is keyed to this epic
   or one of its phases and goes stale the instant that bead closes. For each entry, either resolve the symbol
   (wire it up, privatize it, add a non-test pragma, or delete it per the Symvision epic-whitelist policy) or,
   only when a still-open later bead still needs the exemption, re-key the Justfile line to that open bead. Do not
   leave that judgment for the next agent. `sase bead close` refuses while any of these entries remain. Close the
   epic with `sase bead close sase-18z --note "<what you verified in steps 1-2>"`. After closing, run
...
After the current epic closes, inspect the linked `parent_bead` from `sase bead read sase-18z -r "Need the parent link"`. If there is
...
**Plan file:** `/home/bryan/.sase/plans/202609/bead_note_split_layout.md`

> - **PARENT:** [202609/agent_bead_note_previews.md](202609/agent_bead_note_previews.md)
