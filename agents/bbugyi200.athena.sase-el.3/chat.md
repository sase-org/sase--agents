...
%w:sase-el.2
%w(bead=sase-el.2)
[#bd/work_phase_bead:sase-el.3](https://github.com/sase-org/sase/blob/3ac2b097beac842dc02df1edf88704ff87cd351d/src/sase/default_config.yml#L1020)
...
```markdown
Can you complete the work for bead sase-el.3? The bead is already reserved for
...
description and design file, do the work, and close the bead with `sase bead close sase-el.3 --note "<what you verified>"`. Closing
...
follow-up work as a `PROPOSED FOLLOW-UP:` entry via `sase bead note sase-el.3 'PROPOSED FOLLOW-UP: <one-line summary — detail>'`; the epic's land agent triages these
into task beads.
```
