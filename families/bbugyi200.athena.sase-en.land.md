# Family: sase-en.land

[Agent Hoods](../README.md) / [bbugyi200](../users/bbugyi200/README.md) / [athena](../users/bbugyi200/machines/athena/README.md) / [sase-en](../users/bbugyi200/machines/athena/hoods/sase-en/README.md) / sase-en.land

Owner: `bbugyi200.athena` · Hood: `sase-en` · Members: 2

## Lineage

```mermaid
flowchart TD
  n0["sase-en.land--code [active]"]
  n1["sase-en.land--plan [active]"]
  n0 --> n1
```

The diagram is an optional enhancement; the ordered table below contains the same lineage in accessible text.

| Role | Agent | State | Model / provider | Timing | Commits | Prompt | Chat |
|---|---|---|---|---|---:|---|---|
| <a id="member-code"></a>code | sase-en.land--code | active | gpt-5.5 / codex | 2026-08-03T14:53:46.279423+00:00 | 0 | — | — |
| <a id="member-plan"></a>plan | sase-en.land--plan | active | gpt-5.6-sol / codex | 2026-08-03T14:32:54.667322+00:00 | 0 | [Prompt](../agents/bbugyi200.athena.sase-en.land--plan/prompt.md) | [Chat](../agents/bbugyi200.athena.sase-en.land--plan/chat.md) |

## Neighbors

| Agent | Relation | State |
|---|---|---|
| [sase-en.1](../agents/bbugyi200.athena.sase-en.1/README.md) | sase-en hood | completed |
| [sase-en.2](../agents/bbugyi200.athena.sase-en.2/README.md) | sase-en hood | completed |
| [sase-en.3](../agents/bbugyi200.athena.sase-en.3/README.md) | sase-en hood | completed |
| [sase-en.4](../agents/bbugyi200.athena.sase-en.4/README.md) | sase-en hood | completed |
