# Family: hq.f2.f1

[Agent Hoods](../README.md) / [bbugyi200](../users/bbugyi200/README.md) / [athena](../users/bbugyi200/machines/athena/README.md) / [hq](../users/bbugyi200/machines/athena/hoods/hq/README.md) / hq.f2.f1

Owner: `bbugyi200.athena` · Hood: `hq` · Members: 2

## Lineage

```mermaid
flowchart TD
  n0["hq.f2.f1--code [completed]"]
  n1["hq.f2.f1--plan [active]"]
  n0 --> n1
```

The diagram is an optional enhancement; the ordered table below contains the same lineage in accessible text.

| Role | Agent | State | Model / provider | Timing | Commits | Prompt | Chat |
|---|---|---|---|---|---:|---|---|
| <a id="member-code"></a>code | hq.f2.f1--code | completed | gpt-5.6-sol / codex | 2026-07-22T13:03:50.554081+00:00 | 0 | — | [Chat](../agents/bbugyi200.athena.hq.f2.f1--code/chat.md) |
| <a id="member-plan"></a>plan | hq.f2.f1--plan | active | gpt-5.6-sol / codex | 2026-07-22T12:55:17.277184+00:00 | 0 | [Prompt](../agents/bbugyi200.athena.hq.f2.f1--plan/prompt.md) | [Chat](../agents/bbugyi200.athena.hq.f2.f1--plan/chat.md) |

## Commits

| Role | Repo | Commit | Subject | Committed |
|---|---|---|---|---|
| — | sase | [`d32b2d4`](https://github.com/sase-org/sase/commit/d32b2d4a0ed321c310943e2de5347f1cab021f38) | feat(ace): collapse clans from selected agent panel | 2026-07-22 13:23:52 UTC |

## Neighbors

| Agent | Relation | State |
|---|---|---|
| [hq.f2](bbugyi200.athena.hq.f2.md) (family · 2) | ancestor | active 1, completed 1 |
| [hq](bbugyi200.athena.hq.md) (family · 2) | ancestor | active 1, completed 1 |
| [hq.f2.f0](../agents/bbugyi200.athena.hq.f2.f0/README.md) | hq.f2 hood | active |
| [hq.f0](../agents/bbugyi200.athena.hq.f0/README.md) | hq hood | active |
