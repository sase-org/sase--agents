# Family: gy.f1.f6.f0.w0

[Agent Hoods](../README.md) / [bbugyi200](../users/bbugyi200/README.md) / [athena](../users/bbugyi200/machines/athena/README.md) / [gy](../users/bbugyi200/machines/athena/hoods/gy/README.md) / gy.f1.f6.f0.w0

Owner: `bbugyi200.athena` · Hood: `gy` · Members: 2

## Lineage

```mermaid
flowchart TD
  n0["gy.f1.f6.f0.w0--plan [active]"]
  n1["gy.f1.f6.f0.w0--code [completed]"]
  n0 --> n1
```

The diagram is an optional enhancement; the ordered table below contains the same lineage in accessible text.

| Role | Agent | State | Model / provider | Timing | Commits | Prompt | Chat |
|---|---|---|---|---|---:|---|---|
| <a id="member-plan"></a>plan | gy.f1.f6.f0.w0--plan | active | claude-fable-5 / claude | 2026-07-21T14:33:32.524848+00:00 | 0 | [Prompt](../agents/bbugyi200.athena.gy.f1.f6.f0.w0--plan/prompt.md) | [Chat](../agents/bbugyi200.athena.gy.f1.f6.f0.w0--plan/chat.md) |
| <a id="member-code"></a>code | gy.f1.f6.f0.w0--code | completed | gpt-5.6-sol / codex | 2026-07-21T14:42:17.110482+00:00 | 0 | — | [Chat](../agents/bbugyi200.athena.gy.f1.f6.f0.w0--code/chat.md) |

## Commits

| Role | Repo | Commit | Subject | Committed |
|---|---|---|---|---|
| — | sase | [`d57e220`](https://github.com/sase-org/sase/commit/d57e2207c097dd7fc097f7267e700db5727c3bde) | feat(ace): show model pool effort details | 2026-07-21 11:35:08 EDT |

## Neighbors

| Agent | Relation | State |
|---|---|---|
| [gy.f1.f6.f0](../agents/bbugyi200.athena.gy.f1.f6.f0/README.md) | ancestor | dismissed |
| [gy.f1](../agents/bbugyi200.athena.gy.f1/README.md) | ancestor | completed |
| [gy](../agents/bbugyi200.athena.gy/README.md) | ancestor | completed |
| [gy.f1.f6.f0.w0.f0](../agents/bbugyi200.athena.gy.f1.f6.f0.w0.f0/README.md) | descendant | waiting |
| [gy.f1.f6.f0.w0.f2](bbugyi200.athena.gy.f1.f6.f0.w0.f2.md) (family · 2) | descendant | active 1, completed 1 |
| [gy.f1.f6.f0.w0.f2](../agents/bbugyi200.athena.gy.f1.f6.f0.w0.f2/README.md) | descendant | completed |
| [gy.f1.f6.f0.w0.f2.f1](../agents/bbugyi200.athena.gy.f1.f6.f0.w0.f2.f1/README.md) | descendant | active |
| [gy.f1.f6.f1](../agents/bbugyi200.athena.gy.f1.f6.f1/README.md) | gy.f1.f6 hood | active |
| [gy.f1.f0](../agents/bbugyi200.athena.gy.f1.f0/README.md) | gy.f1 hood | waiting |
| [gy.f1.f1](../agents/bbugyi200.athena.gy.f1.f1/README.md) | gy.f1 hood | waiting |
| [gy.f1.f2](../agents/bbugyi200.athena.gy.f1.f2/README.md) | gy.f1 hood | waiting |
| [gy.f1.f3](../agents/bbugyi200.athena.gy.f1.f3/README.md) | gy.f1 hood | waiting |
| [gy.f1.f4](../agents/bbugyi200.athena.gy.f1.f4/README.md) | gy.f1 hood | active |
| [gy.f1.f5](../agents/bbugyi200.athena.gy.f1.f5/README.md) | gy.f1 hood | active |
| [gy.f1.f7](bbugyi200.athena.gy.f1.f7.md) (family · 2) | gy.f1 hood | active 1, completed 1 |
| [gy.f1.f7](../agents/bbugyi200.athena.gy.f1.f7/README.md) | gy.f1 hood | completed |
| [gy.f1.f8](bbugyi200.athena.gy.f1.f8.md) (family · 2) | gy.f1 hood | active 1, completed 1 |
| [gy.f1.f8](../agents/bbugyi200.athena.gy.f1.f8/README.md) | gy.f1 hood | completed |
| [gy.f1.f8.f0](bbugyi200.athena.gy.f1.f8.f0.md) (family · 2) | gy.f1 hood | active 1, completed 1 |
| [gy.f1.f8.f0](../agents/bbugyi200.athena.gy.f1.f8.f0/README.md) | gy.f1 hood | completed |
| [gy.f0](../agents/bbugyi200.athena.gy.f0/README.md) | gy hood | active |
